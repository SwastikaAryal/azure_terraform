param(
    ...
    [string]$LogFile       = "C:\Temp\HadoopCommon_Update_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
)
...
Write-Log "--- HadoopCommon Update | CVE-2022-26612 | Machine: $env:COMPUTERNAME ---"
...
if ($existingJars) {
    foreach ($j in $existingJars) {
        ...
        if ($j.Name -match 'hadoop-common-([\d\.]+)\.jar') {
...
# Backup
$backupDir = "C:\Temp\HadoopCommon_backup_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
