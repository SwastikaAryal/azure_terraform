...
###############################################################################
# Run 8 – Apply validation (ensure resources are created)
###############################################################################
run "apply_restore_validation" {
  command = apply

  assert {
    condition = azurerm_automation_account.backup_restore.id != ""
    error_message = "Automation account must be created during apply"
  }

  assert {
    condition = azurerm_automation_runbook.full_vm_restore.id != ""
    error_message = "Full VM restore runbook must be created"
  }

  assert {
    condition = azurerm_automation_runbook.disk_restore.id != ""
    error_message = "Disk restore runbook must be created"
  }

  assert {
    condition = azurerm_automation_runbook.file_level_recovery.id != ""
    error_message = "File-level recovery runbook must be created"
  }

  assert {
    condition = azurerm_role_assignment.automation_backup_contributor.id != ""
    error_message = "Backup Contributor role assignment must be created"
  }

  assert {
    condition = azurerm_role_assignment.automation_vm_contributor.id != ""
    error_message = "VM Contributor role assignment must be created"
  }
}