...
Extract and run .\CopyToPSPath.ps1
...
Write-Host "Verifying AD DS configuration..."

try {
    $acct = Get-AzStorageAccount `
        -ResourceGroupName $StorageAccountResourceGroup `
    ...
    if ($adProps) {
        ...
        Write-Warning "AD properties not visible yet — allow a few minutes for propagation."
    }
}
catch {
...
Write-Host ""
Write-Host "Next step: Assign RBAC role to AD users/groups:"
Write-Host "New-AzRoleAssignment -SignInName user@domain -RoleDefinitionName 'Storage File Data SMB Share Contributor' -Scope '/subscriptions/<sub-id>/resourceGroups/$StorageAccountResourceGroup/providers/Microsoft.Storage/storageAccounts/$StorageAccountName'"
